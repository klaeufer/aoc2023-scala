...
object Day4:
  ...
  val example =
    ...
    .stripMargin.split("\n").iterator.map(lineToCard).toIndexedSeq
  ...
  def addCardValues(cards: IndexedSeq[Card]): Int =
    cards.map((w, a) => math.pow(2, w.intersect(a).size - 1).toInt).sum
  ...
  val input = scala.io.Source.fromFile("data/day4Input.txt").getLines().map(lineToCard).toIndexedSeq

  def main(args: Array[String]): Unit =
    println(s"day 4 part 1 example: ${addCardValues(example)}")
    println(s"day 4 part 1 solution: ${addCardValues(input)}")
    println(s"day 4 part 2 example: ${processCards(example)}")
    println(s"day 4 part 2 solution: ${processCards(input)}")
